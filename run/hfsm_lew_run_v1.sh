...
export trkrebd=345.0   # eastern boundary of grid to search for new storms
export trkrwbd=226.0   # western boundary of grid to search for new storms
export trkrnbd=45.0   # northern boundary of grid to search for new storms
export trkrsbd=1.0   # southern boundary of grid to search for new storms
regtype=altg        # An ID to use for scripting to differentiate files for one 
                      search domain ("altg" = Atlantic TC genesis) from another, 
                      like "eptg" (Eastern Pacific TC genesis).
atcffreq=300       # output frequency in the ATCF file for your output track data, in
                   # hours * 100

rundescr="multistorm"   # file_sequence can be equal to either "onebig" for tracker runs
atcfdescr="trkcut"      # in which all your data for all variables & all lead times are 
file_sequence="multi"   # in one big file... I usually run this way for global models where
                        # the grids are stationary and the files are small.  For regional 
                        # models like HAFS & HWRF where there is the possibility of moving
                        # grids from one lead to the next, I run with file_sequence="multi".
                        # In this "multi" mode, there is a separate file for each lead time,
                        # and it needs to be named a certain way, which is usually:
                        # ${model_ID}.${rundescr}.${atcfdescr}.${yyyymmddhh}.f${mmmmm}, where
                        #   model_ID = "hfsm", "hfsb", etc
                        #   rundescr = some descriptive variable / name 
                        #   atcfescr = another descriptive variable / name 
                        #   yyyymmddhh = starting YMDH of your forecast
                        #   mmmmm = lead time of the file, in minutes, in I5.5 format (i.e., 
                        #           must be 5 digits, with leading zeroes if necessary).  The
                        #           run script will create this for you.
                        #
                        # In the script, the grib index files are also created using either 
                        # grbindex (GRIB1) or grb2index (GRIB2).  Those index files must be named
                        # with the same character string as for the GRIB data files, but then 
                        # with ".ix" added after the file name string.

export contour_interval=100.0   # The contour interval (in Pa, not hPa) used for checking for 
                                # a closed contour in the MSLP field.  This checking is only 
                                # done in a genesis run for storms that have been found on the
                                # fly; it's not performed for a standard forward tracking run 
                                # of the tracker that's only looking at RSMC-numbered storms.
max_mslp_850=400.0     # This specifies the maximum allowable distance between the tracker-diagnosed
                       # center fixes for MSLP and 850 mb zeta.  Distance units = km.  This is 
                       # used as one of the checks to determine whether or not tracking should 
                       # continue for a given storm.  When storms start recurving into the 
                       # mid-latitudes and begin to become deformed by the westerlies and become
                       # extratropical, their vertical structure often gets disrupted.  When this
                       # happens, it could be possible for the tracker to "latch onto" a different,
                       # nearby disturbance that is not the storm of interest.  This check helps
                       # prevent that by shutting down tracking if there is too much separation 
                       # between these two lower level center fixes.

mslpthresh=0.0015    # This is used for a very basic MSLP gradient threshold (units = mb/km)that 
                     # must be met in order for tracking to continue for a given storm.
v850thresh=1.5000    # This is used as a basic threshold for the mean tangential, cyclonic wind 
                     # circulation in units of m/s at 850 mb (within something like 225 km or so)
                     # that must be met in order for tracking to continue.
v850_qwc_thresh=1.0000
cint_grid_bound_check=0.50
modtyp='regional'   # Can be either "regional" or "global"
nest_type='fixed'   # Can be either "fixed" or "moveable".  This is used to aid in various checks
                    # for whether or not a moveable grid has stopped moving for too long, 
                    # indicating a potential issue with the model itself having lost the storm.
lead_time_units='hours'   # This can be either "hours" or "minutes".  It has always been "hours" 
                          # for every model I've ever run the tracker on.
export PHASEFLAG=y   # Indicates whether or not to perform testing for thermodynamic structure.
export PHASE_SCHEME=both   # Can be either "cps" (cyclone phase space) or "vtt" (older, Vitart-
                           # style check for a closed contour near the tracker-diagnosed
                           # center fix in the mean 300-500 mb temperature field), or "both"
                           # to perform both checks.  It's important to know that if one or 
                           # both of these checks fail, it will *not* stop the tracker from 
                           # continuing to track a storm.  The phase information is simply 
                           # provided as additional diagnostic output that you can use in a 
                           # postprocessing manner after the tracker runs to determine if/when
                           # a storm that the tracker has found has become tropical.

export WCORE_DEPTH=1.0   # Specifies the depth of the temperature perturbation to check for with
                         # the closed contour check using the "vtt" scheme mentioned in the 
                         # previous item for "PHASE_SCHEME".  Units = deg K.
export STRUCTFLAG=y    # Specifies whether or not (y/n) to diagnose and output additional
                       # wind structure information.
export IKEFLAG=y    # STRUCTFLAG also needs to be set to y, and this IKEFLAG tells the tracker 
                    # to compute Integrated Kinetic Energy stats for storm size (using Mark
                    # Powell's methodology).
export sstflag=y    # Specifies whether or not (y/n) to diagnose the mean, storm-centric SST 
                    # near the tracker-diagnosed fix position.
export shear_calc_flag=y   # Specifies whether or not (y/n) to diagnose 850-200 mb, storm-centric
                           # vertical wind shear.  This uses the same method as the SHIPS scheme,
                           # including the SHIPS method of removing the vortex prior to computing
                           # the shear.
export genflag=y    # Specifies whether or not (y/n) to compute extra diagnostics for genesis, 
                    # stor-centric, including:
                    #   - 850 mb divergence
                    #   - 850 mb moisture convergence
                    #   - Mean RH in the 800-600 mb layer
                    #   - Mean RH in the 1000-925 mb layer
                    #   - Mean 500 mb omega
export use_land_mask=y   # Specifies whether or not (y/n) to use a land mask that will mask out
                         # any land points as potential initial genesis location points.  Use this
                         # whenever possible.
# export read_separate_land_mask_file=y
export read_separate_land_mask_file=n   # Set to y if you need to use a separate land-sea mask that
                                        # is not included in your regular data file.
