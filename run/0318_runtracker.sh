...
#------------------------------------------------------
export PS4=' + run_ecmwf_ez.sh line $LINENO: '

# all the module set up lines below are dealt with in the new runscript
...
export fcsthrs=' 00 06 12 18 24 30 36 42 48 54 60 66 72 78
                 84 90 96 102 108 114 120 126 132 138 144
                 150 156 162 168  99  99  99  99  99  99   99
                  99  99  99  99  99  99  99  99  99  99
                  99  99  99  99  99  99  99  99  99  99
                  99  99  99  99  99  99  99  99  99'

export curymdh=2022092200
...
export HOMEtrak=/mnt/lfs5/HFIP/hfip-gfdl/Caitlyn.Mcallister/usegribdata/currenttracker
export exectrkdir=/mnt/lfs5/HFIP/hfip-gfdl/Caitlyn.Mcallister/usegribdata/currenttracker/exec
export gettrk_exec_dir=${exectrkdir}
export rundir=${HOMEtrak}/run
export workroot=${HOMEtrak}/work

export tcvit_date=/misc/whome/Timothy.Marchok/bin/tcvit_date
export NDATE=/misc/whome/Timothy.Marchok/bin/ndate.x
export gix=grbindex
...
export ymdh=${PDY}${cyc}
export CYL=${cyc} # CAITLYN - this is repetative and is being removed, MIGHT ALSO NEED TO BE EDITED IN NETCDF RUNSCRIPT 
...
# CAITLYN - this was removed from runscript when the cmodel variable was removed
if [ ${trkrtype} == 'tracker' ]; then

  if [ ${cmodel} == 'tshd' ]; then
    ${tcvit_date} ${curymdh} | grep "NHC"                 | \
    grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}'   \
    >${wdir}/vitals.upd.${atcfout}.${curymdh}
  else
  ...
  ${tcvit_date} ${curymdh} | egrep "JTWC|NHC"             | \
    grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}'   \
                 | awk 'substr($0,6,1) !~ /9/ {print $0}'   \
    >${wdir}/vitals.upd.${atcfout}.${curymdh}

fi

num_storms=` cat ${wdir}/vitals.upd.${atcfout}.${curymdh} | wc -l`
...
if [ ${num_storms} -gt 0 ]; then
  ...
  cat ${wdir}/vitals.upd.${atcfout}.${curymdh}
  cat ${wdir}/vitals.upd.${atcfout}.${curymdh} >> ${tcvit_logfile}
  echo " "
  set -x
else
  touch ${wdir}/vitals.upd.${atcfout}.${curymdh}
fi

#------------------------------------------------------------------------
# Set variables & parameters for the input namelist for ECMWF
#------------------------------------------------------------------------
...
g2_jpdtn=0       
inp_data_type=grib
export gribver=1 # not sure why this is =1 and comment in netcdf runscript is =2
g1_mslp_parm_id=151
...
# These next definitions declare the names of variables inside
# NetCDF files.  Since we are using GRIB1 data for ECMWF and *not*
# NetCDF files, we just set these all to the missing value of "X".
...
# CAITLYN - these are not in the netcdf runscript, are these variables needed?
...
/mnt/lfs5/HFIP/hfip-gfdl/Caitlyn.Mcallister/usegribdata/currenttracker/exec/gettrk.x <${namelist}
gettrk_rcc=$?
