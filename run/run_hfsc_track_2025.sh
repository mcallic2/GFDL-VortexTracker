#!/usr/bin/ksh
#PBS -j oe -o /lfs/h2/emc/hur/save/timothy.marchok/trak/para/scripts/hfsc_trak_2025/hfsc_trak_2024092412.out
#PBS -N hfsc_trk
#PBS -A HWRF-DEV
...
export PS4=' + run_hfsc_track_test.sh line $LINENO: '
...
export rundir=/lfs/h2/emc/hur/save/timothy.marchok/trak/para/scripts/hfsc_trak_2025
...
# export hfscdir=/lfs/h1/ops/prod/com/hafs/v1.0/hfsa.${PDY}/${cyc}
export hfscdir=/lfs/h2/emc/ptmp/timothy.marchok/trakout3/2024092412/hfsc
export DATA=${hfscdir}
...
then
  ...
  do
    ...
    then
      ...
      set +x
      echo "+++ TIMING: BEFORE hfsc cut for fhour= $fhour  ---> `date`"
      set -x
    fi
...
#    hfscgfile=${hfscdir}/${stormid}.${ymdh}.hafs.grid01.f${fhour}.grb2
#    hfscgfile=${hfscdir}/${atcfname}.${rundescr}.${atcfdescr}.${gymdh}.f${min5}
    hfsc_orig_gfile=${hfscdir}/${stormid}.${ymdh}.hfsa.parent.trk.f${fhour}.grb2
    hfsc_renamed_gfile=${hfscdir}/${atcfout}.parent.${stormid}.${ymdh}.f${min5}

    if [ -s ${hfsc_orig_gfile} ]
    then
      ln -s -f ${hfsc_orig_gfile} ${hfsc_renamed_gfile}
      $g2ix ${hfsc_renamed_gfile} ${hfsc_renamed_gfile}.ix
    else
      set +x
      echo " "
      echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      echo " !!! hfsc original file missing: ${hfsc_orig_gfile}"
      echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      set -x
      continue
    fi
...
#    set +x
#    echo " "
#    echo " Extracting hfsc GRIB data for forecast hour = $fhour"
#    echo " "
#    set -x
#
#    let min=fhour*60
#    min5=` echo ${min} | awk '{printf ("%5.5d\n",$0)}'`
#
#    hfsccutfile=${DATA}/${atcfname}.${rundescr}.${atcfdescr}.${gymdh}.f${min5}
#
#    >${hfsccutfile}
#
#    echo "before wgrib2 for fhour= $fhour at `date`"
#
#    $wgrib2 -s ${origfile} | egrep "(HGT:900 mb)|(HGT:850 mb)|(HGT:800 mb)|(HGT:750 mb)|(HGT:700 mb)|(HGT:650 mb)|(HGT:600 mb)|(HGT:550 mb)|(HGT:500 mb)|(HGT:450 mb)|(HGT:400 mb)|(HGT:350 mb)|(HGT:300 mb)|(UGRD:850 mb)|(UGRD:700 mb)|(UGRD:500 mb)|(UGRD:200 mb)|(UGRD:10 m )|(VGRD:850 mb)|(VGRD:700 mb)|(VGRD:500 mb)|(VGRD:200 mb)|(VGRD:10 m )|(ABSV:850 mb)|(ABSV:700 mb)|(TMP:500 mb)|(TMP:450 mb)|(TMP:400 mb)|(TMP:350 mb)|(TMP:300 mb)|(MSLET:mean)" | $wgrib2 -i $origfile -append -grib ${hfsccutfile}

#    if [ $fhour -ne 999 ]; then
#      set +x
#      echo "+++ TIMING: AFTER hfsc WGRIB2 for fhour= $fhour  ---> `date`"
#      set -x
#    fi

#    set +x
#    echo " "
#    echo "Time after wgrib2 is `date`"
#    echo " "
#    set -x

    # Now run through the work to get data for phase checking....

#    hfsccutfile_ix=${hfsccutfile}.ix
#    $g2ix ${hfsccutfile} ${hfsccutfile_ix}

#    catfile=${DATA}/${cmodel}.${PDY}${CYL}.f${fhour}.catfile
#    >${catfile}
...
#      ln -s -f ${rundir}/hfsc_hgt_levs.txt                fort.16
...
#
#      ln -s -f ${gfile}                                   fort.11
#      ln -s -f ${rundir}/hfsc_tmp_levs.txt                fort.16
#      ln -s -f ${ifile}                                   fort.31
...
namelist=${DATA}/input.hfsc_track_test
