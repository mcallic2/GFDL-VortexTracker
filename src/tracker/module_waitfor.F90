module module_waitfor
  ...
  integer(kind=8), parameter :: minage_def = 30, minsize_def = 1, maxwait_def = -1, sleeptime_def = 3

  contains

  subroutine run_command(command, retval)
  ! This routine uses the C system command to run a command.
  ! This gets around any arbitrary command length limits in fortran.
  ! Also, this way we get the command return status.
    ...
    call run_cmd_helper (trim(command)//char(0), len_trim(command)+1, retval)
  ...
  subroutine waitfor(filename, status, minage, minsize, maxwait, sleeptime)
    ...
    call waitfor_helper (status, minage_in, minsize_in, maxwait_in, sleeptime_in, trim(filename), len_trim(filename))

  end subroutine waitfor

  subroutine waitfor_helper (status, minage, minsize, maxwait, sleeptime, filename, n)
    ...
    call cwaitfor (status, minage, minsize, maxwait, sleeptime, filename0)
