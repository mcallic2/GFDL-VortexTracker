module def_vitals
  ...
  type tcvcard         ! Define a new type for a TC Vitals card
    character(len=4) :: tcv_center      ! Hurricane Center Acronym
    character(len=4) :: tcv_storm_id    ! Storm Identifier (03L, etc)
    character(len=9) :: tcv_storm_name  ! Storm name
    ...
    character(len=1) :: tcv_latns       ! 'N' or 'S'
    integer          :: tcv_lon         ! Storm Lon (*10), always >0
    character(len=1) :: tcv_lonew       ! 'E' or 'W'
    ...
    character(len=1) :: tcv_depth       ! Storm depth (S,M,D) X=missing
  end type tcvcard
...
module gen_vitals
  ...
  type gencard     ! Define a new type for a genesis vitals card
    ...
    character(len=1) :: gv_gen_latns   ! 'N' or 'S'
    integer          :: gv_gen_lon     ! genesis lon (*10), always >0
    character(len=1) :: gv_gen_lonew   ! 'W' or 'E'
    character(len=3) :: gv_gen_type    ! 'FOF'; or TC Vitals ATCF ID
    ...
    character(len=1) :: gv_obs_latns   ! 'N' or 'S'
    integer          :: gv_obs_lon     ! Storm Lon (*10), always >0
    character(len=1) :: gv_obs_lonew   ! 'E' or 'W'
    ...
    character(len=1) :: gv_depth       ! Storm depth (S,M,D) X=missing
    end type gencard
...
module inparms
  ...
  type datecard  ! Define a new type for the input namelist parms
    ...
    character(len=8) :: modtyp ! 'global' or 'regional'
    character(len=7) :: lt_units ! 'hours' or 'minutes' to indicate the
                            ! units of lead times in grib files
    character(len=6) :: file_seq ! 'onebig' or 'multi' tells if grib
                                 ! data will be input as one big file or
                                 ! as individual files for each tau.
    character(len=8) :: nesttyp ! Either "moveable" or "fixed"
  end type datecard
...
module trkrparms
  ...
  type trackstuff  ! Define a new type for various tracker parms
    ...
    character(len=7) :: type    ! 'tracker', 'midlat' or 'tcgen'
    real             :: mslpthresh ! min mslp gradient to be maintained
    character(len=1) :: use_backup_mslp_grad_check ! If a mslp fix could
    ...
    character(len=1) :: use_backup_850_vt_check ! If an 850 mb wcirc fix
                                 ...
                                 ! Has a value of 'y' or 'n'.
    character(len=8) :: gridtype ! 'global' or 'regional'
    ...
    character(len=1) :: out_vit  ! Flag for whether to write out vitals
    character(len=1) :: use_land_mask ! Flag used only in tcgen tracking
                                 ...
                                 ! Has a value of 'y' or 'n'.
    character(len=1) :: read_separate_land_mask_file ! Flag that says 
                                 ! whether or not a separate file will
                                 ! be read in that contains the land-
                                 ! sea mask.  Has a value of 'y' or 'n'.
    character(len=6) :: inp_data_type ! Has a value of 'grib' or 
                                      ! 'netcdf'
...
module atcf
  ...
  character(len=4) :: atcfname  ! ATCF Name of model (GFSO for GFS...)
...
module gfilename_info
  ...
  character(len=4), save  :: gmodname  ! Model ID for first part of GRIB
                                  ! file name ("gfdl","hwrf","hrs", etc)
  character(len=40), save :: rundescr ! This is descriptive and up to
                                  ! the developer (e.g., "6thdeg",
                                  ! "9km_run","1.6km_run"         
                                  ! "15km_ens_run_member_n13", etc)
  character(len=40), save :: atcfdescr ! This is optional.  If used, it
...
module phase
  ...
  character(len=1), save ::   phaseflag   ! Will phase be determined
                                            ! (y/n)                   
  character(len=4), save ::   phasescheme ! What scheme to use:
...
module structure
  ...
  character(len=1), save ::   structflag  ! Will structure be
                                            ! analyzed (y/n)?
  character(len=1), save ::   ikeflag     ! Will IKE & SDP be
                                            ! computed (y/n)?
...
module shear_diags
  ...
  character(len=1), save :: shearflag   ! Will vertical shear
                                            ! be analyzed (y/n)?
end module shear_diags

module sst_diags

  character(len=1), save :: sstflag     ! Will SST 
                                            ! be analyzed (y/n)?
end module sst_diags

module genesis_diags

  character(len=1), save :: genflag        ! Will genesis diags
                                               ! be analyzed and
                                               ! reported (y/n)?
  character(len=1), save :: gen_read_rh_fields ! Will RH fields be
                                               ! read in directly (y/n)?
  character(len=1), save :: need_to_compute_rh_from_q ! Will spec.
                                               ! humidity (q) fields be
                                               ! read in to compute RH
                                               ! if RH is not read in?
                                               ! (y/n)
  character(len=1), save :: smoothe_mslp_for_gen_scan ! Did 
...
module waitfor_parms
  ...
  character(len=1)           :: use_waitfor ! y or n, for waiting for input files
  ...
  character(len=1)           :: use_per_fcst_command ! enable per_fcst_command
  character(len=pfc_cmd_len) :: per_fcst_command ! command to run every forecast time
...
module netcdf_parms
  ...
  type netcdfstuff  ! Define a new type for NetCDF information
    ...
    integer ::   num_netcdf_vars ! Total *possible* 
                                  ...
                                  ! are not.
    character(len=180) :: netcdf_filename ! character file name for 
                                           ! the NetCDF file.
    character(len=180) :: netcdf_lsmask_filename ! character file name
                                      ! for the optional, separate 
                                      ! NetCDF file if the user has 
                                      ! indicated this with the
                                      ! read_separate_land_mask_file
                                      ! flag.
    character(len=30) ::  rv850name  ! 850 mb rel vort
    character(len=30) ::  rv700name  ! 700 mb rel vort
    character(len=30) ::  u850name   ! 850 mb u-comp
    character(len=30) ::  v850name   ! 850 mb v-comp
    character(len=30) ::  u700name   ! 700 mb u-comp
    character(len=30) ::  v700name   ! 700 mb v-comp
    character(len=30) ::  z850name   ! 850 mb gp height
    character(len=30) ::  z700name   ! 700 mb gp height
    character(len=30) ::  mslpname   ! mslp
    character(len=30) ::  usfcname   ! near-sfc u-comp
    character(len=30) ::  vsfcname   ! near-sfc v-comp
    character(len=30) ::  u500name   ! 500 mb u-comp
    character(len=30) ::  v500name   ! 500 mb v-comp
    character(len=30) ::  tmean_300_500_name  ! Mean Temp in
                                               ! 300-500 mb layer
    character(len=30) ::  z500name   ! 500 mb gp height
    character(len=30) ::  z200name   ! 200 mb gp height
    character(len=30) ::  lmaskname  ! Land mask
    character(len=30) ::  z900name   ! 900 mb gp height
    character(len=30) ::  z800name   ! 800 mb gp height
    character(len=30) ::  z750name   ! 750 mb gp height
    character(len=30) ::  z650name   ! 650 mb gp height
    character(len=30) ::  z600name   ! 600 mb gp height
    character(len=30) ::  z550name   ! 550 mb gp height
    character(len=30) ::  z450name   ! 450 mb gp height
    character(len=30) ::  z400name   ! 400 mb gp height
    character(len=30) ::  z350name   ! 350 mb gp height
    character(len=30) ::  z300name   ! 300 mb gp height
    character(len=30) ::  time_name  ! Name of time variable,
                                      ! usually "time"
    character(len=30) ::  lon_name   ! longitudes
    character(len=30) ::  lat_name   ! latitudes
    character(len=30) ::  time_units ! "days" or "hours"
    character(len=30) ::  u200name   ! 200 mb u-comp
    character(len=30) ::  v200name   ! 200 mb v-comp
    character(len=30) ::  sstname    ! SST
    character(len=30) ::  q850name   ! 850 mb specific humidity
    character(len=30) ::  rh1000name ! 1000 mb RH
    character(len=30) ::  rh925name  ! 925 mb RH
    character(len=30) ::  rh800name  ! 800 mb RH
    character(len=30) ::  rh750name  ! 750 mb RH
    character(len=30) ::  rh700name  ! 700 mb RH
    character(len=30) ::  rh650name  ! 650 mb RH
    character(len=30) ::  rh600name  ! 600 mb RH
    character(len=30) ::  spfh1000name ! 1000 mb specific humidity
    character(len=30) ::  spfh925name ! 925 mb specific humidity
    character(len=30) ::  spfh800name ! 800 mb specific humidity
    character(len=30) ::  spfh750name ! 750 mb specific humidity
    character(len=30) ::  spfh700name ! 700 mb specific humidity
    character(len=30) ::  spfh650name ! 650 mb specific humidity
    character(len=30) ::  spfh600name ! 600 mb specific humidity
    character(len=30) ::  temp1000name ! 1000 mb temperature
    character(len=30) ::  temp925name ! 925 mb temperature
    character(len=30) ::  temp800name ! 800 mb temperature
    character(len=30) ::  temp750name ! 750 mb temperature
    character(len=30) ::  temp700name ! 700 mb temperature
    character(len=30) ::  temp650name ! 650 mb temperature
    character(len=30) ::  temp600name ! 600 mb temperature
    character(len=30) ::  omega500name ! 500 mb omega
